...
testa_rede=`echo "SELECT cod_cliente FROM tbl_clientes WHERE rede_cliente=\"$1\"" | mysql -u${usuario} -p${senha} CTRL_DTCOM | grep -v cod_cliente`
...
#			cp -l "$dir_cliente/$esqueleto" "$dir_pacote_temp/$esqueleto"
		...
		cod_cliente=`echo "SELECT cod_cliente FROM tbl_clientes WHERE rede_cliente=\"$1\"" | mysql -u${usuario} -p${senha} CTRL_DTCOM | grep -v cod_cliente`

		#obtem valor da ultima sequencia para o codigo da rede do cliente
		sequencia=`echo "SELECT sequencia FROM tbl_seq_pacotes WHERE cod_cliente=\"$cod_cliente\"" | mysql -u${usuario} -p${senha} CTRL_DTCOM | grep -v sequencia`

		#soma 1 na sequencia dos pacotes (novo pacote)
		sequencia=$((sequencia+1))

		#insere no banco a nova seqquencia do pacote
		echo "UPDATE tbl_seq_pacotes SET sequencia=\"$sequencia\" WHERE cod_cliente=\"$cod_cliente\"" | mysql -u${usuario} -p${senha} CTRL_DTCOM
		...
		echo "INSERT INTO tbl_pacotes (cod_cliente,seq_pacote,tarja_pacote,nome_pacote,tamanho_pacote,dt_hora_criacao,status) VALUES (\"$cod_cliente\",\"$sequencia\",\"$data\",\"$nome_pacote\",\"$tamanho_pacote\",\"$data_mysql\",1)" | mysql -u${usuario} -p${senha} CTRL_DTCOM

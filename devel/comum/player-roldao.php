...
if($streaming == true)
{
    ...
    exec("mplayer -slave -nolirc -softvol -idle -quiet -noautosub -input file=$in_stream 2>&- 1> $out_stream &"); //abre mplayer q toca stream

    $cmd = "echo 'set_property volume ".$volumeMusical."' > /tmp/stream_in";
    exec($cmd);
...
exec("mplayer -slave -nolirc -softvol -idle -quiet -noautosub -input file=$in_comerciais 2>&- 1> $out_comerciais &"); //abre mplayer q toca comerciais

$cmd = "echo 'set_property volume ".$volumeComercial."' > /tmp/comerciais_in";
exec($cmd);
